...
import (
	...
	"github.com/agex-labs/orynx/abci/strategies/codec"
	"github.com/agex-labs/orynx/abci/strategies/currencypair"
	orynxabci "github.com/agex-labs/orynx/abci/types"
...
func GetOracleVotes(
	...
	if len(proposal) < orynxabci.NumInjectedTxs {
		return nil, orynxabci.MissingCommitInfoError{}
	}

	extendedCommitInfo, err := extCommitCodec.Decode(proposal[orynxabci.OracleInfoIndex])
	if err != nil {
		return nil, orynxabci.CodecError{
...
type VoteAggregator interface {
	...
	AggregateOracleVotes(ctx sdk.Context, votes []Vote) (map[orynxtypes.CurrencyPair]*big.Int, error)
	...
	GetPriceForValidator(validator sdk.ConsAddress) map[orynxtypes.CurrencyPair]*big.Int
}

func NewDefaultVoteAggregator(
	logger log.Logger,
	aggregateFn aggregator.AggregateFnFromContext[string, map[orynxtypes.CurrencyPair]*big.Int],
...
type DefaultVoteAggregator struct {
	...
	priceAggregator *aggregator.DataAggregator[string, map[orynxtypes.CurrencyPair]*big.Int]
...
func (dva *DefaultVoteAggregator) AggregateOracleVotes(ctx sdk.Context, votes []Vote) (map[orynxtypes.CurrencyPair]*big.Int, error) {
...
func (dva *DefaultVoteAggregator) addVoteToAggregator(ctx sdk.Context, address string, oracleData vetypes.OracleVoteExtension) error {
	...
	prices := make(map[orynxtypes.CurrencyPair]*big.Int, len(oracleData.Prices))
	for cpID, priceBz := range oracleData.Prices {
		if len(priceBz) > orynxabci.MaximumPriceSize {
			dva.logger.Debug(
...
func (dva *DefaultVoteAggregator) GetPriceForValidator(validator sdk.ConsAddress) map[orynxtypes.CurrencyPair]*big.Int {
