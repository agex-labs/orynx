...
import (
	...
	"github.com/agex-labs/orynx/abci/strategies/aggregator/mocks"
	"github.com/agex-labs/orynx/abci/strategies/codec"
	"github.com/agex-labs/orynx/abci/testutils"
	abcimocks "github.com/agex-labs/orynx/abci/types/mocks"
...
func TestPriceApplier(t *testing.T) {
	...
	t.Run("ignore negative prices", func(t *testing.T) {
		...
		cp := orynxtypes.NewCurrencyPair("BTC", "USD")
		va.On("AggregateOracleVotes", ctx, []aggregator.Vote{
			{
				OracleVoteExtension: vetypes.OracleVoteExtension{
					Prices: prices,
	...
	t.Run("update prices in state", func(t *testing.T) {
		...
		ok.On("GetAllCurrencyPairs", ctx).Return(
			[]orynxtypes.CurrencyPair{cp, orynxtypes.NewCurrencyPair("ETH", "USD")}, // ignore last cp
		)
		...
		require.NoError(t, err)
		require.Equal(t, map[orynxtypes.CurrencyPair]*big.Int{
			cp: big.NewInt(150),
		}, prices)

		// get prices from validators
		expPrices := map[orynxtypes.CurrencyPair]*big.Int{
			cp: big.NewInt(150),
		}
