...
type DeltaCurrencyPairStrategy struct {
	*DefaultCurrencyPairStrategy
	cache          map[orynxtypes.CurrencyPair]*big.Int
...
func NewDeltaCurrencyPairStrategy(oracleKeeper OracleKeeper) *DeltaCurrencyPairStrategy {
	return &DeltaCurrencyPairStrategy{
		...
		cache:                       make(map[orynxtypes.CurrencyPair]*big.Int, DefaultCacheInitialCapacity),
	}
}
...
func (s *DeltaCurrencyPairStrategy) getOnChainPrice(ctx sdk.Context, cp orynxtypes.CurrencyPair) (*big.Int, error) {
	height := ctx.BlockHeight()
	if height != s.previousHeight {
		s.cache = make(map[orynxtypes.CurrencyPair]*big.Int, DefaultCacheInitialCapacity)
