...
type DefaultCurrencyPairStrategy struct {
	...
	idCache        map[uint64]orynxtypes.CurrencyPair
...
func NewDefaultCurrencyPairStrategy(oracleKeeper OracleKeeper) *DefaultCurrencyPairStrategy {
	strategy := &DefaultCurrencyPairStrategy{
		oracleKeeper: oracleKeeper,
		idCache:      make(map[uint64]orynxtypes.CurrencyPair, DefaultCacheInitialCapacity),
	}
	return strategy
}
...
func (s *DefaultCurrencyPairStrategy) ID(ctx sdk.Context, cp orynxtypes.CurrencyPair) (uint64, error) {
	// reset cache if the block height has changed
	height := ctx.BlockHeight()
	if height != s.previousHeight {
		s.idCache = make(map[uint64]orynxtypes.CurrencyPair, DefaultCacheInitialCapacity)
...
func (s *DefaultCurrencyPairStrategy) FromID(ctx sdk.Context, id uint64) (orynxtypes.CurrencyPair, error) {
	// reset cache if the block height has changed
	height := ctx.BlockHeight()
	if height != s.previousHeight {
		s.idCache = make(map[uint64]orynxtypes.CurrencyPair, DefaultCacheInitialCapacity)
	...
	if !found {
		return orynxtypes.CurrencyPair{}, fmt.Errorf("id %d not found", id)
	}
