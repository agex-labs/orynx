...
func (s *HashCurrencyPairStrategy) ID(ctx sdk.Context, cp orynxtypes.CurrencyPair) (uint64, error) {
...
func (s *HashCurrencyPairStrategy) FromID(ctx sdk.Context, id uint64) (orynxtypes.CurrencyPair, error) {
	// reset cache if the block height has changed
	height := ctx.BlockHeight()
	if height != s.previousHeight {
		s.idCache = make(map[uint64]orynxtypes.CurrencyPair, DefaultCacheInitialCapacity)
	...
	for _, cp := range allCPs {
		hash, err := CurrencyPairToHashID(cp.String())
		if err != nil {
			return orynxtypes.CurrencyPair{}, fmt.Errorf("failed to hash currency pair %s: %w", cp.String(), err)
		}
	...
	cp, found = s.idCache[id]
	if !found {
		return orynxtypes.CurrencyPair{}, fmt.Errorf("currency pair with sha256 hashed ID %d not found in x/oracle state", id)
	}
