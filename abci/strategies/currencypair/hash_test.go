...
import (
	...
	strategies "github.com/agex-labs/orynx/abci/strategies/currencypair"
	"github.com/agex-labs/orynx/abci/strategies/currencypair/mocks"
...
func TestHashCurrencyPairStrategyID(t *testing.T) {
	...
	t.Run("test a currency pair with no base/quote", func(t *testing.T) {
		ok.On("GetIDForCurrencyPair", mock.Anything, orynxtypes.CurrencyPair{}).Return(uint64(0), false).Once()

		// expect an error when querying for a currency-pair not in module-state
		id, err := strategy.ID(ctx, orynxtypes.CurrencyPair{})
		require.Error(t, err)
...
func TestHashCurrencyPairStrategyFromID(t *testing.T) {
	...
	t.Run("test getting currency pair for currency pair that does not exist in state", func(t *testing.T) {
		ok.On("GetAllCurrencyPairs", ctx).Return([]orynxtypes.CurrencyPair{}).Once()
	...
	t.Run("test getting currency pair for currency pair that exists in state (no cache)", func(t *testing.T) {
		ok.On("GetAllCurrencyPairs", ctx).Return([]orynxtypes.CurrencyPair{btcusd}).Once()
