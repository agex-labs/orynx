...
import (
	...
	preblock "github.com/agex-labs/orynx/abci/preblock/oracle"
	compression "github.com/agex-labs/orynx/abci/strategies/codec"
	codecmock "github.com/agex-labs/orynx/abci/strategies/codec/mocks"
	"github.com/agex-labs/orynx/abci/strategies/currencypair"
	currencypairmock "github.com/agex-labs/orynx/abci/strategies/currencypair/mocks"
	"github.com/agex-labs/orynx/abci/testutils"
	"github.com/agex-labs/orynx/abci/types"
	orynxabcimocks "github.com/agex-labs/orynx/abci/types/mocks"
	vetypes "github.com/agex-labs/orynx/abci/ve/types"
	"github.com/agex-labs/orynx/aggregator"
	"github.com/agex-labs/orynx/pkg/math/voteweighted"
	voteweightedmocks "github.com/agex-labs/orynx/pkg/math/voteweighted/mocks"
	orynxtypes "github.com/agex-labs/orynx/pkg/types"
	servicemetrics "github.com/agex-labs/orynx/service/metrics"
	metricmock "github.com/agex-labs/orynx/service/metrics/mocks"
	"github.com/agex-labs/orynx/x/oracle/keeper"
...
func (s *PreBlockTestSuite) SetupTest() {
	...
	s.currencyPairs = []orynxtypes.CurrencyPair{
		{
			Base:  "BTC",
			Quote: "ETH",
		},
...
func (s *PreBlockTestSuite) TestPreBlockStatus() {
	s.Run("failure - nil request", func() {
		...
		handler := preblock.NewOraclePreBlockHandler(
			log.NewTestLogger(s.T()),
			func(_ sdk.Context) aggregator.AggregateFn[string, map[orynxtypes.CurrencyPair]*big.Int] {
				return func(_ aggregator.AggregatedProviderData[string, map[orynxtypes.CurrencyPair]*big.Int]) map[orynxtypes.CurrencyPair]*big.Int {
					return nil
				}
			},
			nil,
	...
	s.Run("too many price bytes causes no errors in aggregating currency pairs", func() {
		...
		mockOracleKeeper := orynxabcimocks.NewOracleKeeper(s.T())
		handler := preblock.NewOraclePreBlockHandler(
			log.NewTestLogger(s.T()),
			func(_ sdk.Context) aggregator.AggregateFn[string, map[orynxtypes.CurrencyPair]*big.Int] {
				return func(_ aggregator.AggregatedProviderData[string, map[orynxtypes.CurrencyPair]*big.Int]) map[orynxtypes.CurrencyPair]*big.Int {
...
func (s *PreBlockTestSuite) TestValidatorReports() {
	...
	s.Run("test that if a validator's report is absent, this is recorded", func() {
		...
		mogUsd := orynxtypes.NewCurrencyPair("MOG", "USD")

		handler := preblock.NewOraclePreBlockHandler(
			log.NewTestLogger(s.T()),
			func(_ sdk.Context) aggregator.AggregateFn[string, map[orynxtypes.CurrencyPair]*big.Int] {
				return func(_ aggregator.AggregatedProviderData[string, map[orynxtypes.CurrencyPair]*big.Int]) map[orynxtypes.CurrencyPair]*big.Int {
					return map[orynxtypes.CurrencyPair]*big.Int{
		...
		mockOracleKeeper.On("GetAllCurrencyPairs", s.ctx).Return([]orynxtypes.CurrencyPair{btcUsd, mogUsd}, nil)
