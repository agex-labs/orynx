...
import (
	...
	"github.com/agex-labs/orynx/abci/ve"
...
func (h *ProposalHandler) PrepareProposalHandler() sdk.PrepareProposalHandler {
	return func(ctx sdk.Context, req *cometabci.RequestPrepareProposal) (resp *cometabci.ResponsePrepareProposal, err error) {
		...
		// report the orynx specific PrepareProposal latency
		defer func() {
			...
			h.logger.Debug(
				...
				"orynx prepare proposal latency", (totalLatency - wrappedPrepareProposalLatency).Seconds(),
			)

			orynxabci.RecordLatencyAndStatus(h.metrics, totalLatency-wrappedPrepareProposalLatency, err, servicemetrics.PrepareProposal)
		}()

		if req == nil {
			h.logger.Error("PrepareProposalHandler received a nil request")
			err = orynxabci.NilRequestError{
		...
		if err != nil {
			...
			err = orynxabci.WrappedHandlerError{
				Handler: servicemetrics.PrepareProposal,
				Err:     err,
			}
	...
	return func(ctx sdk.Context, req *cometabci.RequestProcessProposal) (resp *cometabci.ResponseProcessProposal, err error) {
		...
		defer func() {
			...
			h.logger.Debug(
				...
				"orynx prepare proposal latency", (totalLatency - wrappedProcessProposalLatency).Seconds(),
			)
			orynxabci.RecordLatencyAndStatus(h.metrics, totalLatency-wrappedProcessProposalLatency, err, servicemetrics.ProcessProposal)
		}()
		...
		if voteExtensionsEnabled {
			...
			if !h.retainOracleDataInWrappedHandler {
				injectedTx = req.Txs[orynxabci.OracleInfoIndex]
				req.Txs = req.Txs[orynxabci.NumInjectedTxs:]
			}
		}
