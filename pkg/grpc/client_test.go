...
func TestClient(t *testing.T) {
	...
	t.Run("try dialing via non supported GRPC target URL (i.e tcp prefix)", func(t *testing.T) {
		...
		client, err := orynxgrpc.NewClient(fmt.Sprintf("tcp://localhost:%s", port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	...
	t.Run("try dialing via supported GRPC target URL (i.e host:port)", func(t *testing.T) {
		...
		client, err := orynxgrpc.NewClient(fmt.Sprintf("localhost:%s", port), grpc.WithTransportCredentials(insecure.NewCredentials()))
		require.NoError(t, err)
