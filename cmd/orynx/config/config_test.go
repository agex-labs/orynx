...
import (
	...
	cmdconfig "github.com/agex-labs/orynx/cmd/orynx/config"
	oracleconfig "github.com/agex-labs/orynx/oracle/config"
...
func TestReadOracleConfigWithOverrides(t *testing.T) {
	...
	t.Run("overriding variables from environment", func(t *testing.T) {
		...
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_UPDATEINTERVAL", updateIntervalOverride.String())
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_METRICS_PROMETHEUSSERVERADDRESS", prometheusServerOverride)
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_PROVIDERS_RAYDIUM_API_API_ENDPOINTS_1_URL", endpointOverride.URL)
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_PROVIDERS_RAYDIUM_API_API_ENDPOINTS_1_AUTHENTICATION_APIKEY", endpointOverride.Authentication.APIKey)
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_PROVIDERS_RAYDIUM_API_API_ENDPOINTS_1_AUTHENTICATION_APIKEYHEADER", endpointOverride.Authentication.APIKeyHeader)
		t.Setenv(cmdconfig.OrynxConfigEnvironmentPrefix+"_PROVIDERS_COINBASE_WS_WEBSOCKET_ENDPOINTS_0_URL", endpointOverride.URL)
	...
	t.Run("overriding variables via config", func(t *testing.T) {
		...
		tmpfile, err := os.CreateTemp("", "orynx-config-*.json")
		require.NoError(t, err)
