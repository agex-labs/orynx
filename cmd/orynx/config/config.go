...
import (
	...
	"github.com/agex-labs/orynx/cmd/constants"
	"github.com/agex-labs/orynx/oracle/config"
	mmtypes "github.com/agex-labs/orynx/service/clients/marketmap/types"
)

const (
	// DefaultUpdateInterval is the default value for how frequently orynx updates aggregate price responses.
	DefaultUpdateInterval = 250000000
	// DefaultMaxPriceAge is the default value for the oldest price considered in an aggregate price response by orynx.
	DefaultMaxPriceAge = 120000000000
	// DefaultPrometheusServerAddress is the default value for the prometheus server address in orynx.
	DefaultPrometheusServerAddress = "0.0.0.0:8002"
	// DefaultMetricsEnabled is the default value for enabling prometheus metrics in orynx.
	DefaultMetricsEnabled = true
	// DefaultHost is the default for the orynx oracle server host.
	DefaultHost = "0.0.0.0"
	// DefaultPort is the default for the orynx oracle server port.
	DefaultPort = "8080"
	// jsonFieldDelimiter is the delimiter used to separate fields in the JSON output.
	jsonFieldDelimiter = "."
	// OrynxConfigEnvironmentPrefix is the prefix for environment variables that override the orynx config.
	OrynxConfigEnvironmentPrefix = "ORYNX_CONFIG"
)

// DefaultOracleConfig returns the default configuration for the orynx oracle.
...
func setViperDefaultsForDataStructure(keyPrefix string, config interface{}) {
	...
	viper.SetEnvPrefix(OrynxConfigEnvironmentPrefix)
	viper.AutomaticEnv()
