...
import (
	...
	"github.com/agex-labs/orynx/x/oracle/types/mocks"
)
...
func (s *KeeperTestSuite) TestGetNumRemovedCurrencyPairs() {
	...
	s.Run("get 1 with 1 remove", func() {
		...
		s.Require().NoError(s.oracleKeeper.CreateCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin1"}))
		s.Require().NoError(s.oracleKeeper.RemoveCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin1"}))
	...
	s.Run("get 2 with 2 removes", func() {
		...
		s.Require().NoError(s.oracleKeeper.CreateCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin1"}))
		s.Require().NoError(s.oracleKeeper.RemoveCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin1"}))
		s.Require().NoError(s.oracleKeeper.CreateCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin2"}))
		s.Require().NoError(s.oracleKeeper.RemoveCurrencyPair(s.ctx, orynxtypes.CurrencyPair{Base: "test", Quote: "coin2"}))
...
func (s *KeeperTestSuite) TestGetCurrencyPairMapping() {
	cp1 := orynxtypes.CurrencyPair{Base: "TEST", Quote: "COIN1"}
	cp2 := orynxtypes.CurrencyPair{Base: "TEST", Quote: "COIN2"}
	...
	s.Run("get 1 with 1 cp", func() {
		...
		mapping, err := s.oracleKeeper.GetCurrencyPairMapping(s.ctx)
		s.Require().NoError(err)
		s.Require().Equal(map[uint64]orynxtypes.CurrencyPair{
			0: cp1,
		}, mapping)
	})
