...
import (
	...
	marketmaptypes "github.com/agex-labs/orynx/x/marketmap/types"
...
func (s *KeeperTestSuite) TestGetAllCurrencyPairs() {
	...
	s.Run("after CurrencyPairs are registered, all of them are returned from the query", func() {
		...
		s.Require().NoError(s.oracleKeeper.CreateCurrencyPair(s.ctx, orynxtypes.CurrencyPair{
			Base:  "AA",
			Quote: "BB",
		}))
		s.Require().NoError(s.oracleKeeper.CreateCurrencyPair(s.ctx, orynxtypes.CurrencyPair{
		...
		s.Require().NoError(s.oracleKeeper.SetPriceForCurrencyPair(s.ctx, orynxtypes.CurrencyPair{
			Base:  "GG",
			Quote: "HH",
	...
	tcs := []struct {
			...
			func() {
				s.mockMarketMapKeeper.On("GetMarket", mock.Anything, mock.Anything).Return(marketmaptypes.Market{
					Ticker: marketmaptypes.Ticker{
						CurrencyPair:     orynxtypes.CurrencyPair{Base: "CC", Quote: "BB"},
						Decimals:         8,
						MinProviderCount: 3,
						Metadata_JSON:    "",
					},
		...
		},
		{
			...
			func() {
				s.mockMarketMapKeeper.On("GetMarket", mock.Anything, mock.Anything).Return(marketmaptypes.Market{
					Ticker: marketmaptypes.Ticker{
						CurrencyPair:     orynxtypes.CurrencyPair{Base: "AA", Quote: "ETHEREUM"},
						Decimals:         18,
						MinProviderCount: 3,
						Metadata_JSON:    "",
					},
...
func (s *KeeperTestSuite) TestGetCurrencyPairMappingListGRPC() {
	...
	cp3 := orynxtypes.CurrencyPair{Base: "TEST", Quote: "COIN3"}
