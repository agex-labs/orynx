...
import (
	...
	"github.com/agex-labs/orynx/x/marketmap/types/tickermetadata"
)

func Test_UnmarshalAgEx(t *testing.T) {
	t.Run("can marshal and unmarshal the same struct and values", func(t *testing.T) {
		elem := tickermetadata.NewAgEx(
			100,
			1000,
		...
		bz, err := tickermetadata.MarshalAgEx(elem)
		require.NoError(t, err)

		elem2, err := tickermetadata.AgExFromJSONBytes(bz)
		require.NoError(t, err)
	...
	t.Run("can marshal and unmarshal the same struct and values with empty AggregatorIDs", func(t *testing.T) {
		elem := tickermetadata.NewAgEx(100, 1000, nil, false)
	...
	t.Run("can unmarshal a JSON string into a struct", func(t *testing.T) {
		...
		elem, err := tickermetadata.AgExFromJSONString(elemJSON)
		require.NoError(t, err)

		require.Equal(t, tickermetadata.NewAgEx(
			100,
			1000,
			[]tickermetadata.AggregatorID{
				tickermetadata.NewAggregatorID("coingecko", "id"),
