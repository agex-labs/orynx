...
and consumed by a set of oracle service providers (Orynx oracle, etc.).
...
message Ticker {
  ...
  orynx.types.v1.CurrencyPair currency_pair = 1
...
message ProviderConfig {
  ...
  orynx.types.v1.CurrencyPair normalize_by_pair = 3;
...
```shell
grpcurl -plaintext localhost:9090 orynx.marketmap.v1.Query/MarketMap
```
...
```shell
grpcurl -plaintext localhost:9090 orynx.marketmap.v1.Query/LastUpdated
```
...
```shell
grpcurl -plaintext localhost:9090 orynx.marketmap.v1.Query/Params
```
...
```shell
  orynxd q marketmap last-updated
```
