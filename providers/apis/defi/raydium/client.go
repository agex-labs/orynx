...
import (
	...
	orynxhttp "github.com/agex-labs/orynx/pkg/http"
...
func solanaClientFromEndpoint(endpoint config.Endpoint) (*rpc.Client, error) {
	opts := []orynxhttp.HeaderOption{
		orynxhttp.WithOrynxVersionUserAgent(),
	}

	// if authentication is enabled
	if endpoint.Authentication.Enabled() {
		// add authentication header
		opts = append(opts, orynxhttp.WithAuthentication(endpoint.Authentication.APIKeyHeader, endpoint.Authentication.APIKey))
	}

	transport := orynxhttp.NewRoundTripperWithHeaders(http.DefaultTransport, opts...)
