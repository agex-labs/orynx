...
// APIHandler implements the MarketMapAPIDataHandler interface for the aGeX prices module, which can be used
// by a base provider. This is specifically for fetching market data from the aGeX prices module, which is
// then translated to a market map.
type APIHandler struct {
	logger *zap.Logger

	// api is the api config for the aGeX market params API.
...
// a single market map object that was converted from the aGeX market params response.
func (h *APIHandler) ParseResponse(
	...
	if resp == nil {
		h.logger.Debug("got nil response from agex market params API")
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
	...
	// Parse the response body into a agex market params response object.
	var params agextypes.QueryAllMarketParamsResponse
	if err := json.NewDecoder(resp.Body).Decode(&params); err != nil {
		h.logger.Debug("failed to parse agex market params response", zap.Error(err))
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to parse agex market params response: %w", err),
				providertypes.ErrorFailedToDecode,
			),
		)
	}

	// Convert the agex market params to a market map.
	marketResp, err := ConvertMarketParamsToMarketMap(params)
	if err != nil {
		h.logger.Debug(
			"failed to convert agex market params to market map",
		...
		return types.NewMarketMapResponseWithErr(
			chains,
			providertypes.NewErrorWithCode(
				fmt.Errorf("failed to convert agex market params to market map: %w", err),
