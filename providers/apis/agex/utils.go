...
const (
	...
	// SwitchOverAPIHandlerName is the name of the aGeX switch over API.
	SwitchOverAPIHandlerName = "agex_migration_api"

	// ResearchAPIHandlerName is the name of the aGeX research json API.
	ResearchAPIHandlerName = "agex_research_json_api"

	// ResearchCMCAPIHandlerName is the name of the aGeX research json API that only returns CoinMarketCap markets.
	ResearchCMCAPIHandlerName = "agex_research_coinmarketcap_api"

	// ChainID is the chain ID for the aGeX market map provider.
	ChainID = "agex-node"

	// Endpoint is the endpoint for the aGeX market map API.
	Endpoint = "%s/agex-labs/prices/params/market?pagination.limit=10000"
...
// DefaultAPIConfig returns the default configuration for the aGeX market map API.
...
// DefaultSwitchOverAPIConfig returns the default configuration for the aGeX switch over API provider.
...
// DefaultResearchAPIConfig returns the default configuration for the aGeX market map API.
var DefaultResearchAPIConfig = config.APIConfig{
	...
	Endpoints: []config.Endpoint{
		{
			URL: "https://agex-api.lavenderfive.com",
		},
		{
			URL: "https://raw.githubusercontent.com/agex-labs/agex-web/main/public/configs/otherMarketData.json",
		},
	},
}

// DefaultResearchCMCAPIConfig returns the default configuration for the aGeX market map API that only returns CoinMarketCap markets.
...
// UniswapV3MetadataFromTicker returns the metadataJSON string for uniswapv3_api according to the aGeX encoding.
...
// All raydium tickers on agex will be formatted as follows
