...
import (
	...
	"github.com/agex-labs/orynx/providers/apis/bitstamp"
	...
	"github.com/agex-labs/orynx/providers/apis/kraken"
...
// ProviderMapping is referencing the different providers that are supported by the aGeX market params.
//
// ref: https://github.com/agex-labs/agex/blob/main/protocol/daemons/pricefeed/client/constants/exchange_common/exchange_id.go
...
// ConvertMarketParamsToMarketMap converts a aGeX market params response to a orynx market map response.
...
// CreateTickerFromMarket creates a ticker from a aGeX market.
func CreateTickerFromMarket(market agextypes.MarketParam) (mmtypes.Ticker, error) {
...
// CreateCurrencyPairFromPair creates a currency pair from a aGeX market.
func CreateCurrencyPairFromPair(pair string) (orynxtypes.CurrencyPair, error) {
	split := strings.Split(pair, Delimiter)
	if len(split) != 2 {
		return orynxtypes.CurrencyPair{}, fmt.Errorf("expected pair (%s) to have 2 elements, got %d", pair, len(split))
	}

	cp := orynxtypes.NewCurrencyPair(
...
// from a aGeX market. These paths represent the different ways to convert a currency
// pair using the aGeX market.
func ConvertExchangeConfigJSON(
	...
	var (
		...
		seen      = make(map[agextypes.ExchangeMarketConfigJson]struct{})
	)
	...
	for _, cfg := range config.Exchanges {
		...
		// This means we have seen an exchange that orynx cannot support.
		...
		var normalizeByPair *orynxtypes.CurrencyPair
...
func ExtractMetadata(providerName string, cfg agextypes.ExchangeMarketConfigJson) (string, error) {
...
func ConvertDenomByProvider(denom string, exchange string) (string, error) {
	...
	case exchange == raydium.Name:
		...
		return orynxtypes.NewCurrencyPair(fields[0], fields[1]).String(), nil
	default:
		return denom, nil
	}
}
