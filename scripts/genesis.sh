...
./build/orynxd init validator --chain-id skip-1 --home "$HOMEDIR"
./build/orynxd keys add validator --home "$HOMEDIR" --keyring-backend test
./build/orynxd genesis add-genesis-account validator 10000000000000000000000000stake --home "$HOMEDIR" --keyring-backend test
./build/orynxd genesis add-genesis-account cosmos1see0htr47uapjvcvh0hu6385rp8lw3em24hysg 10000000000000000000000000stake --home "$HOMEDIR" --keyring-backend test
./build/orynxd genesis gentx validator 1000000000stake --chain-id skip-1 --home "$HOMEDIR" --keyring-backend test
./build/orynxd genesis collect-gentxs --home "$HOMEDIR"
...
for i in $(env | grep -v "_BALANCE" | grep -o "GENESIS_ACCOUNT_[0-9]*" | sort -n -t _ -k 3); do
    ...
    ./build/orynxd genesis add-genesis-account $ACCOUNT $BALANCE --home "$HOMEDIR" --keyring-backend test
done
