...
Connect prices are stored within the [x/oracle](https://github.com/agex-labs/orynx/tree/main/x/oracle) module.

On very specific chains - right now only aGeX - they live in a different storage module (in aGeX's case, `x/prices`).
...
Connect's market configuration is stored in the [x/marketmap](https://github.com/agex-labs/orynx/tree/main/x/marketmap). This module, unlike `x/oracle`, does **not** store price data. Instead, it stores which currency pairs are supported and how they are configured.
...
1. (REST): `curl http://localhost:1317/orynx/marketmap/v1/marketmap`
2. (application cli): `appd q marketmap marketmap`
3. (gRPC): `grpcurl -plaintext localhost:9090 orynx.marketmap.v1.Query/MarketMap`
...
1. (REST): `curl http://localhost:1317/orynx/oracle/v1/get_prices`
2. (gRPC): `grpcurl -plaintext localhost:9090 orynx.oracle.v1.Query/GetPrices`
