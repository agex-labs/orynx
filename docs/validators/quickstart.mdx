...
import QuickStartAGEXSnippet from '/snippets/agex-quickstart.mdx';
...
```bash
curl -sSL https://raw.githubusercontent.com/agex-labs/orynx/main/scripts/install.sh | sudo bash
```

### From GitHub Releases

Head over to our [GitHub Releases](https://github.com/agex-labs/orynx/releases) page and download the binary that matches your machine's architecture.
...
```shell
git clone git@github.com:agex-labs/orynx.git
cd orynx
...
<Tabs>
    ...
    <Tab title="aGeX Blockchain">
        <QuickStartAGEXSnippet />
    </Tab>
...
```shell
curl 'http://localhost:8080/orynx/oracle/v1/prices' | jq .
```
