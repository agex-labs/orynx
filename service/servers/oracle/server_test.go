...
import (
	...
	"github.com/agex-labs/orynx/oracle/mocks"
	...
	"github.com/agex-labs/orynx/service/metrics"
	server "github.com/agex-labs/orynx/service/servers/oracle"
	stypes "github.com/agex-labs/orynx/service/servers/oracle/types"
...
func (s *ServerTestSuite) TestOracleServerPrices() {
	...
	httpResp, err := s.httpClient.Get(fmt.Sprintf("http://%s:%s/orynx/oracle/v1/prices", localhost, port))
	s.Require().NoError(err)
	...
	dummyMarketMap := mmtypes.MarketMap{Markets: map[string]mmtypes.Market{
		"foo": {
			Ticker: mmtypes.Ticker{
				CurrencyPair:     orynxtypes.CurrencyPair{Base: "ETH", Quote: "USD"},
				Decimals:         420,
				MinProviderCount: 79,
				Enabled:          true,
				Metadata_JSON:    "",
